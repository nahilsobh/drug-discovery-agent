...
for RedClaw. The agent runs a Claude-powered loop (`run_agent.py`) calling
...
Outputs: structured JSON audit trail + branded RedClaw PDF report per run.
...
tools/
  ...
  discovery.py        — find_gaps, get_biology, search_redclaw_trials, find_combinations,
...
knowledge_base/
  ...
  redclaw_pipeline.json        — RedClaw pipeline assets (59 assets)
...
python3.11 run_agent.py "Find gaps in RedClaw's oncology pipeline"
...
Output: `reports/RedClaw_vs_<company>_<date>.pdf`
...
| `redclaw_pipeline.json` | 59 RedClaw pipeline assets | ~26KB |
...
- `knowledge_base/` files contain RedClaw-internal strategic analysis — treat as confidential. Do not log or transmit outside the sHPC environment.
...
Built in collaboration with **[redclaw.dev](https://redclaw.dev)**. The GenomeClaw
GPU API (`fold_target`, `score_variant_effect`, `predict_admet`, `cluster_scaffolds`,
`dock_compound`) is redclaw.dev's software — source at
`https://git.redclaw.dev/genomeclaw/genomeclaw`. With thanks to the redclaw.dev team
